...
test_that("collapses categories correctly for Mendelian Peas example", {
  ...
  output_total_explicit <- multBayesInformed(factor_levels=factor_levels, 
                                             Hr=Hr1, a=a, x=counts,  
...
test_that("collapses categories correctly for ordered binomials", {
  ...
  output_total_explicit <- binomBayesInformed(factor_levels=factor_levels, 
  ...
  expect_equal(output_total_explicit$bf_list,
    list(
      bf_type = "BFre",
      bf = structure(
