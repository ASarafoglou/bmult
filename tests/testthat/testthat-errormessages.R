...
test_that("incorrect specification of factor levels", {
  ...
  expect_error(multBfInformed(factor_levels=factor_levels, Hr=c('roundYellow < notAFactor = roundGreen < wrinkledGreen'), a=a, x=counts),
               "\nThe following factor level(s) are invalid: notAFactor \n", fixed = TRUE)
  # using same factor name multiple times
  expect_error(multBfInformed(factor_levels=factor_levels, Hr=c('roundYellow < wrinkledYellow = roundGreen < wrinkledGreen < roundYellow'), a=a, x=counts),
               "Do not use factor levels multiple times within the order restriction.", fixed = TRUE)
  # using opposing restriction signs
  expect_error(multBfInformed(factor_levels=factor_levels, Hr=c('roundYellow < wrinkledYellow = roundGreen > wrinkledGreen'), a=a, x=counts),
