...
For multinomial models, assuming that all category probabilities in a model are equality constrained, the Bayes factor \(\text{BF}_{0e}\) is defined as:
\begin{align*}
\text{BF}_{0e} =  \frac{
...
\begin{table}[H]
	\centering
	\caption{Observed Counts, Observed Proportions, and Expected Proportions of first digits in Greece governmental data. The total sample size was $N = 1{,}497$ observations. Note that the observed proportions and counts deviate slightly from those reported in Rauch et al. (2011) (probably due to rounding errors).}
...
Here, we are testing the null hypothesis against the following four alternative hypotheses:
\begin{align*}
...
\end{align*}
We can also compare the four alternative hypothesis directly with each other or use posterior model probabilities \cite{berger2005posterior} to compare the relative plausibility of the four hypotheses. To compare two informed hypotheses with each other, we can make use of the transitivity property of the Bayes factor. For instance, if we would like to compare \(\mathcal{H}_{0}\) with \(\mathcal{H}_{r1}\), we would first compute \(\text{BF}_{e0}\) and \(\text{BF}_{er1}\) and then yield \(\text{BF}_{0r1}\) by dividing the two quantities:
\[\text{BF}_{0r1} = \cfrac{\text{BF}_{e0}}{\text{BF}_{er1}}.\] To compute the posterior model probability, for instance, for \(\mathcal{H}_0\), we can use the following formula assuming that all hypotheses are equally likely \emph{a priori}:
\[
p(\mathcal{H}_0 \mid \mathbf{x}) = \cfrac{\text{BF}_{0e}}{\text{BF}_{0e} + \text{BF}_{r1e} + \text{BF}_{r2e} + \text{BF}_{r3e}} .
\]

### Method
We can compute \(\text{BF}_{0e}\) and \(\text{BF}_{r2e}\) by means of a Bayesian multinomial test which is implemented in the function \texttt{mult\_bf\_equality}. For that, we only need to specify (1) a vector with observed counts, (2) a vector with concentration parameters of the Dirichlet prior distribution, and (3) the vector of expected proportions. We do not incorporate any specific expectations about the distribution of leading digits in the Greek fiscal data. Hence, we set all concentration parameters to one which corresponds to a uniform Dirichlet distribution.
...
Since the hypotheses \(\mathcal{H}_{r1}\) and \(\mathcal{H}_{r3}\)  contain inequality constraints, we use the function \texttt{mult\_bf\_informed} to compute the Bayes factors  \(\text{BF}_{r1e}\) and \(\text{BF}_{r3e}\). In this function, we need to specify (1) a vector with observed counts, (2) the informed hypothesis \(\mathcal{H}_{r1}\) or \(\mathcal{H}_{r3}\) (e.g., as character vector), (3) a vector with concentration parameters of the Dirichlet prior distribution, and (4) labels for the categories of interest (i.e., leading digits):
...
```
We now make use of the transitivity property of the Bayes factor to compare all alternative hypotheses to the null hypothesis \(\mathcal{H}_{0}\). We also compute the posterior model probabilities for all hypotheses.
...
\begin{table}[H]
	\centering
	\caption{Bayes Factors and Posterior Model Probabilies Resulting From the Four Bayesian Adaptations of the Benford Test.}
	\begin{tabular}{ccc}
		\hline Alternative Hypothesis & $\text{log}(\text{BF}_{.0})$ & $p(\mathcal{H}_{.} \mid \mathbf{x})$ \\
		\hline
		$\mathcal{H}_{0}$  &  $0$  &
		`r papaja::printnum(post_probs[1, 2], digits = 2, format = 'e')`\\
		$\mathcal{H}_{e}$  &
...
The results of the four adaptations of the Benford test are summarized in Table \ref{Tab:benford}. As the evidence is extreme in all four cases, all Bayes factors are reported on the log scale. We can make the following statements concerning the comparison of the null hypothesis to the four alternative hypotheses. The first Bayes factor suggests extreme evidence \textit{in favor of} the hypothesis that the first digits vary freely; \(\text{log}(\text{BF}_{e0}) =\) `r bayes_factors[1, 2]`. The second Bayes factor suggests extreme evidence \textit{in favor of} the hypothesis that the first digits follow a decreasing trend, \(\text{log}(\text{BF}_{r10}) =\) `r bayes_factors[2, 2]`. The third Bayes factor suggests extreme evidence \textit{against} the hypothesis that all first digits occur equally often, \(\text{log}(\text{BF}_{r20}) =\) `r bayes_factors[3, 2]`. The fourth Bayes factor suggests extreme evidence \textit{against} the hypothesis that the first digits follow a fraudulent pattern, \(\text{log}(\text{BF}_{r30}) =\) `r bayes_factors[4, 2]`. When we look at the posterior model probabilities, we can see that \(\mathcal{H}_{r1}\) receives almost the full share of the posterior probability mass, with \(p(\mathcal{H}_{r1} \mid \mathbf{x}) =\) `r papaja::printnum(post_probs[3, 2], digits = 4, format = 'f')`. The hypothesis that receives the smallest portion of the posterior probability mass is \(\mathcal{H}_{r3}\) with \(p(\mathcal{H}_{r3} \mid \mathbf{x}) =\) `r papaja::printnum(post_probs[4, 2], digits = 2, format = 'e')`.
...
In order to get a clearer picture about the ordering of the journals, we can investigate the posterior estimates under the encompassing model as the next step. The posterior median and 95\% credible interval are returned by the \texttt{summary}-method and can be plotted, Figure\ \@ref(fig:journals).
