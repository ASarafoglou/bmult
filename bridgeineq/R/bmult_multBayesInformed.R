...
#' Runs full Analysis
#'
#' @param factor_levels matrix of dimension (nsamples x nparams) with samples from truncated Dirichlet density
...
multBayesInformed <- function(factor_levels, Hr, a, counts = NULL, niter = 5e4, bf_type = 'LogBFer', seed=NULL, ...){
  ...
  .checkNrParameters(factor_levels, alpha = a, data = counts)
  ...
  # Encode H_r
  restrictions          <- generateRestrictionList(Hr, constrained_factors, a, counts)
  ...
  if(!purrr::is_empty(equalities$hyp)){
    ...
    for(i in seq_along(equalities$equality_hypotheses)){
      ...
      equalities_list[[i]] <- multBayesBfEquality(alphas_equalities, counts_equalities, thetas)
  ...
  if(!purrr::is_empty(inequalities$hyp)){
    ...
    for(i in seq_along(inequalities$inequality_hypotheses)){
      ...
      if(prior_is_uniform & !any_free_parameters & no_collapsed_categories){
        ...
        bs_results[[i]]              <- multBayesBfInequality(prior.samples[[i]], restrictions=inequalities, index=index, prior=TRUE, seed=seed)
      ...
      bs_results[[i]]             <- multBayesBfInequality(post.samples[[i]], restrictions=inequalities, index=index, seed=seed)
